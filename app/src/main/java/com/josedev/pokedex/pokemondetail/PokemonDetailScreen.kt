...
import com.josedev.pokedex.util.*
import java.util.*
...
fun PokemonDetailScreen(
    ...
    val pokemonInfo = produceState<ResourceClass<Pokemon>>(initialValue = ResourceClass.Loading()) {
    ...
    )
    {
        ...
        Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.TopCenter) {
            if (pokemonInfo is ResourceClass.Success) {
                pokemonInfo.data?.sprites?.let {
                    Image(
                        painter = rememberCoilPainter(request = it.front_default, fadeIn = true),
...
fun PokemonDetailStateWrapper(
    pokemonInfo: ResourceClass<Pokemon>,
    modifier: Modifier = Modifier,
    loadingModifier: Modifier = Modifier
) {
    when (pokemonInfo) {
        is ResourceClass.Success -> {
            PokemonDetailSection(
                pokemonInfo = pokemonInfo.data!!,
                modifier = modifier.offset(y = (-20).dp)
            )
        }
        is ResourceClass.Error -> {
            Text(
    ...
    Row(
        ...
        for (type in types) {
            Box(
                ...
                Text(
                    text = parseTypeToSpanish(type),//type.type.name.capitalize(Locale.ROOT)
                    color = Color.White,
                    fontSize = 18.sp
                )
            }
